      ...
      this.redirect_uris.forEach((redirectUri) => {
        ...
        switch (this.application_type) { // eslint-disable-line default-case
          case 'web': {
            ...
            if (!configuration.features.dangerouslyEnableLocalhost && this.grant_types.includes('implicit') && protocol === 'http:') {
              invalidate('redirect_uris for web clients using implicit flow MUST only register URLs using the https scheme');
            }

            if (
              !configuration.features.dangerouslyEnableLocalhost
              && !configuration.features.dangerouslyAllowLocalhostRedirectOnImplicitFlow
              && this.grant_types.includes('implicit')
              && hostname === 'localhost'
            ) {
